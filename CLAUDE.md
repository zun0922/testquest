...
- 🔄 **AL-TTA 進行中（第1・2章完成）**：server背景（設定書v1.5）＋`al-tta-1/`（1本）`al-tta-2/`（5本）完成（監修承認・2026-07-04）。ミニワーク〔カバレッジ計算〕・K4〔技法選択〕・匠のみ登場・server背景初活用。index **総計56本**（FL27＋AL-TM23＋AL-TTA6）。単体114＋E2E8 PASS・実画面PO共有済み
- 🔲 **AL-TTA 残り15本**：第3章 静的解析と動的解析（3本）・第4章 テクニカルテストのための品質特性（6本）・第5章 レビュー（3本）・第6章 テストツールと自動化（3本）。完成で **AL-TTA全21本→企画書Phase 3全コンテンツ達成**
- 🔲 **その他候補**：通しプレイFB反映（確認シート回収待ち・本番URLでもプレイ可）／類似性逆引き（B-4・server背景も対象追加）／画像圧縮（pngquant・34MB→約1/4）
