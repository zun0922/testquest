...
- 🔲 **次の候補**：①**AL-TTA 着手**（21本。まず server背景の制作〔設定書§5.2予約・image-asset-production.skill〕→第1章起案。メンター=匠が先発）②通しプレイFB反映（確認シート回収待ち・本番URLでのプレイも可に）③類似性逆引き（B-4）④画像圧縮（pngquant・34MB→約1/4）
