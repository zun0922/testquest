...
## 5. 開発フロー（正＝`docs/開発フロー_役割分担定義_v0.1.md` **v1.0 承認済み**。本節はその要約）
...
> 🎉 **MVP（Phase 1）完了・2026-08-21 PO承認**（`docs/MVP完了報告書_v0.1.md` **v1.0**）
> 実績＝**シナリオ71本**（FL27/AL-TM23/AL-TTA21・15章・569ノード・**決定ポイント280問すべて3択**・good63=7.5%・参照シラバス項番151）／**単体177件＋E2E15件 全PASS**・CI 全成功・`npm audit` 0件／画像27枚2.06MB（WebP）／**本番稼働** https://testsim-three.vercel.app
> **リリース判定：可能**（PO判断）。⚠ **AC-011 は仕様変更**（縦画面完結→横持ち前提＋回転案内）＝**要件定義書 v1.1 で反映済み**（2026-08-25・POレビュー待ち）／⚠ **102TCの人間による機能テストは未実施**（E2E＋全数監修＋通しプレイで代替。TD群は整備済みで着手可能）／⚠ B-4②法務確認未了での公開はPOリスク了承・社内パイロット限定
> **今後の課題（PO判断）：** 問題文・ストーリー構成の細かい調整／102TC手動テスト／~~要件定義書 v1.1 のPOレビュー~~（**2026-09-13 承認済み**）／Phase 2 以降（D系）

- ✅ **キャラクターボイス（FR-P2-006・Phase 2）＝本番稼働中**（2026-08-25 社内FB→2026-08-30 **PR #1 マージ・本番反映**）：方式＝**VOICEVOX 0.25.2 CPU版**（無料・CPU推論・決定的生成・ユーザー辞書で読みを制御）
  - **確定配役（PO選定・規約確認済み）**：narration=東北イタコ／rin=**冥鳴ひまり**／tanaka=**玄野武宏**／ken=白上虎太郎／takumi=麒ヶ島宗麟／mio=波音リツ（**シナリオ登場0のため未生成**）。設定の正＝`docs/音声設定書_v0.1.md`（v0.2）・判定根拠＝`docs/音声採用記録_v0.1.md`（v0.2）・規約原文＝`docs/音声ライブラリ規約一覧_v0.1.md`
  - **全量生成済み（2026-08-30）**：**71本・569ノード・70.4分・26.09MB**（実測。ビルド成果物 `dist/audio` は28MB）。単体**205**＋E2E**19** 全PASS（門番7件が全71本を検証）
  - 生成は**6〜7秒/ノード**・章単位で分割実行（全量で約50分）。**1シナリオごとに manifest を保存**するため中断しても続きから再開できる。5xxは最大3回リトライ
  - ⚠ **メモリに注意**：物理7.7GB環境で空き0.4GBまで逼迫すると `/synthesis` 500・Node OOM が発生する。**エンジン再起動で回復**
  - **クレジット表記は規約必須**。manifest の `credits`（実際に配信している音源のみ）を `TitleScreen` が表示し、**E2Eで表示を担保**
  - ⚠ 生成には**エンジンの起動が必要**（`%LOCALAPPDATA%/Programs/VOICEVOX/vv-engine/run.exe` をヘッドレス起動。winget ではなく zip 手動展開のため winget 管理外）
  - **本番スモーク合格**：全71シナリオの音声配信を全数確認（NG 0件）・本番 manifest はローカルと完全一致・`audio/mp4` で配信
  - **本プロジェクト初のPR運用**（`feature/character-voice` → PR #1 → merge commit `58bc981`）。⚠ **CI はブランチ push では走らない**（トリガーが `push: branches:[main]` と `pull_request:`）＝**PRを作って初めて起動する**
  - A-6（クリック挙動）・A-7（疑似コードの読み）は**PO確認のうえ現状維持で決着**。変更したくなれば辞書追加＋`--force` で該当分のみ鳴らし直せる（費用ゼロ）
  - リポジトリサイズ 35.4MB → **約66MB**（音声26MB。AACのため `git gc` の効果は小さい）
  - 設計の要点：**シナリオJSON 71本は不変**（`public/audio/{scenarioId}/{nodeId}.m4a` の規約でパス解決）／音声が無い・OFF・再生拒否でも**従来どおりプレイできる**／`manifest.json` のSHA-256で**差分再生成**／門番テストがシナリオとの食い違いを検出
  - ⚠ **配信形式は AAC(m4a)**＝Ogg Opus は iOS 16 以前の Safari が再生できないため（当初案から変更）
  - ⚠ AL-TTA に**疑似コードを含むセリフが5件**（読み上げの扱いは STEP 3 でPO判断）
- ✅ **ヒント機能（FR-P2-007）＝本番稼働中**（2026-08-30 PO提案→要件確定→実装→全章展開→**PR #2 マージ・本番反映**）：蓄積ステータスが何にも使われていない課題への対応。**選択肢の重要語を強調**するヒントを**4種の合計ポイント**に応じて**任意表示**する。詳細の正＝`docs/ヒント機能_要件仕様_v0.1.md`（実測：開始40／best は50本目で上限400／poor は71本で183）。**`Choice.emphasis` を追加し `text` は不変**・章単位で段階導入可。**未決4点はPO判断済み**（宿題A-8）
  - **構成＝ヒント文が主役・強調は補助**（2026-08-30 PO確認「強調だけでは微妙。テキストの方が親切」を反映）。**Lv1＝ヒント文／Lv2＝ヒント文＋全選択肢の強調**
  - **実装済み**：`utils/hint.ts`（レベル別閾値 FL 60/150・AL 170/300）／`ChoiceNode.hint?: string`（最大100字・**#14**）＋`Choice.emphasis?: string[]`（**本文は不変**・**#13**）／ChoiceOverlay のヒントUI／`tailwind` に `hint: #7ce0ff`。どちらのデータも**省略可**で片方だけでも動く
  - **全章展開済み**：**280問すべてに `hint`**／**840選択肢中837に `emphasis`**（未付与3件＝`al-tta-3-01/q2` は選択肢が「3」「2」で強調に適さないため意図的に省略）。**71ファイル・1,117行追加・削除0＝本文は一切不変**
  - 挿入は `scripts/apply-hints.mjs`（**行ベース挿入で既存整形を保つ**・事前に全件検証して違反があれば何も書かない）。単体**223**＋E2E**24** 全PASS。要件仕様 **v0.4**（**書き方ガイドライン §4.2**）／設計書 **v1.7**
  - **本番スモーク合格**：全71シナリオを全数検証＝280問すべてに `hint`／強調語840件がすべて本文に実在／**不整合0件**
  - 🔲 **残＝実プレイでのヒント文の粒度確認**（特に AL 編。気になる箇所はデータを直すだけで差し替え可能＝コード変更不要）
- ✅ **エンディング（FR-P2-002・Phase 2）＝本番稼働中**（2026-08-30 PR #3 マージ・本番反映）：**全7種**（FL4種＝ステータス分岐／AL-TM・AL-TTA 各1種／両ルート制覇で最終1種）。到達済みは**一覧から再生**できる（未到達は「？？？」）
  - ⚠ **企画書 §7.2 の条件は現データでは機能しなかった**（実測：どのプレイでも知識が100に張り付き、全問poorですら「知識の探求者」／隠しENDは到達不可）。原因＝**poor 選択肢の加算の89%が知識**
  - 解決＝**poor で得た分を判定から除外**した達成率で判定（3ステータスの差が8ポイント未満なら隠しEND）。ランダム8件が4種に分散することを実測で確認
  - `ClearRecord.cleanGain`・`SaveData.endings` を**省略可で追加**（既存セーブと後方互換）。判定の理論最大値は**門番テストがシナリオデータと突き合わせる**＝データ改訂でCIが落ちる
  - 単体**246**＋E2E**30** 全PASS。要件仕様＝`docs/エンディング機能_要件仕様_v0.1.md`（v0.3）／設計書 **v1.8**／要件定義書 **v1.3**
- ✅ **既読スキップ・バックログ（FR-P2-005・Phase 2）＝本番稼働中**（2026-09-06 PR #5 マージ・本番反映）：再プレイの利便性向上（MVP見送り分）
  - **既読スキップ**＝一度読んだノードだけ自動送り。**未読・選択肢・フィードバック・中断・画面操作で必ず止まる**（読み飛ばしと出題スキップを防ぐ＝学習教材としての線引き）。止まる条件は**1か所の副作用に集約**
  - **既読はノード単位**で記録（シナリオ単位では分岐の未通過枝まで既読になる）。保存は **`testquest:read`＝進捗とは別キー**（壊れても学習進捗を失わない・読めなければ未読扱いで通常どおり遊べる）
  - **バックログ**＝現在のシナリオで読んだセリフを読み返せる。**音声のある行に再生ボタン**（既存の音声をそのまま使うので追加制作は不要）
  - 単体**257**＋E2E**37** 全PASS。設計書 **v1.9**／要件定義書 FR-P2-005 を実装内容へ更新
- ✅ **編成パターン（FR-P2-003・Phase 2）＝本番稼働中**（2026-09-13 PR #6 マージ・本番反映。**本番スモーク合格**＝澪144本/既定569本の配信・切替で立ち絵/名前/名乗りが入れ替わる・不整合0件）：**技術メンター役1役の差し替え**（高橋 匠 ⇄ 伊藤 澪）。**追加キャラの新規制作は不要**（澪は元々「匠の女性案」で立ち絵5表情を整備済み）＝**宿題A-4 解消**
  - **シナリオJSON 71本は不変**。差し替えるキャラと本文差分は `public/data/casting.json`。**本文差分は7か所だけ**（匠のセリフ144本のうち男性寄りの表現は5%・名乗りは全71本で1か所という実測に基づく）
  - **性別は尋ねない**（PO決定）。「技術メンターを選べます」と直接選ぶ方式＝体験は同じで**性別情報を保持しない**（企画書§9.5）。変更はシナリオ選択画面＝章の区切りのみ（§9.1）
  - **音声＝澪 144ノード・16.4分・6.0MB**（波音リツ・規約で他キャラへの声当て可を明示）。`public/audio/mio/` に分離し**既定の26MBには触れない**＝パターンを増やしても増分は差し替え分だけ。生成は `node scripts/gen-voice.mjs --cast mio`
  - 門番テスト＝立ち絵5表情の実在／差分の宛先ノードが実在し対象キャラのセリフ／**旧キャラの名前が残っていない**／**名前が出るセリフの差分付け忘れ**／音声144本の充足と話者の一致
  - 単体**280**＋E2E**44** 全PASS。設計書 **v2.0**／要件定義書 FR-P2-003・004 を更新
- ✅ **文書整合（Phase 2）完了・2026-09-13 PO一括承認**：要件定義書 **v1.5**／テスト計画書 **v1.4**／テスト仕様書 **v1.5**／テスト手順書 **v1.4**（依頼書＝`docs/文書整合レビュー依頼_Phase2_v0.1.md` v1.0）
  - ⚠ **発見した乖離**：Phase 2 の5機能は全て本番稼働済みなのに、**テスト計画書は「未実装フェーズの機能」・テスト仕様書は「本書では扱わない」と明記**したままだった（約3週間）。品質ではなく**文書だけ**が実態から取り残されていた
  - **受け入れ基準 AC-013〜017 を新設**（要件定義書 §11.2）＝本番稼働中の5機能に合格基準が存在しなかったため。基準は「実装済みの挙動」をそのまま明文化（後から作ると判断がぶれる）
  - テスト仕様書 §4 に **Phase 2 の機能TC 31件**（自動30／人間1）・**§5.9 に手動TC 7件（ST-M2）**・**§8.3 に自動化カバレッジ**（機械が見る範囲と人が見る範囲の対応）・§9 に AC→TC を追加。手順書 §5.9 に手動7件の手順
  - **人手の増分は7件だけ**＝実機の音声再生／音声とテキストの並行／読み上げ品質／**澪の声と口調の相性**／ヒント文の粒度／エンディング本文の読み味／スキップの体感速度。自動と同じことを人手で繰り返さない方針
  - **教訓（テスト計画書 §2.2 に恒久化）**：機能を本番へ出すときは**テスト文書の対象範囲も同じタイミングで動かす**。要件定義書に FR を足すだけではテスト系文書は「対象外」のまま残る
- ✅ **Phase 2 手動テスト（ST-M2 7件）実施完了**（2026-09-13・**PO実機**／リモート一問一答）＝**PASS 6／要改善 1／FAIL 0**。記録＝`docs/テスト実施記録_ST-M2_v0.1.md`（v0.2）
  - **設計の前提は実機で成立**：音声が使えなくても遊べる／読み飛ばさせない／数値を出さない
  - ⚠ **E2Eでは絶対に拾えない指摘が2件出た**＝手動テストを7件に絞った価値がここにあった
  - ✅ **課題#2 スキップ速度＝完了**（2度の是正）。①一律90ms「速すぎる」→②260ms+8ms/字「**連打（200〜330ms）より遅く実感がない**」→③**140ms+4ms/字（中央値約300ms）＋モード表示＋停止理由**でPASS確定
    - **教訓**：速度調整は「遅くすれば直る」ではない。**人が手で操作する速さが基準線**／モードの有無は**操作している場所（本文の真上）**で示す。画面隅のボタンの色だけでは伝わらない
    - 副次的に発見：**スキップ中は音声を鳴らさない**ように変更（従来は1秒に約11回、声が鳴っては切られていた）
  - 🔲 **課題#1 澪の丁寧語化＝未着手**（PO決定）。本文差分 **7か所→144か所**・音声再生成144本（約16分・無料）・**144本の監修が必要**。シナリオJSON 71本は変更不要
  - ⚠ **スマホ単体では AL編・エンディングに到達できない**（本番は `?unlockAll=1` 無効＝正しい状態）。今回は**音声ファイルの聞き比べ**と**スクリーンショット**で代替確認した
  - 単体**288**＋E2E**49** 全PASS
- 📌 **旧記載（履歴）：要件定義書 v1.1・v1.2 起案・POレビュー待ち**（2026-08-25。v1.0 は 2026-06-13 承認）＝実装・確定仕様との整合（宿題C-3）：**AC-011 を横持ち前提＋回転案内へ**（設計書 v1.3 のPO承認が上位文書に未反映だった）／**AC-003・AC-012 の対象を実績71本へ**（当初計画は併記して残す）／実装値の差分（原則3択・キャラ5名・WebP・エラー画面・データ駆動の章表示）。**連鎖確認で下流も同時改版**＝テスト計画書 v1.2・テスト仕様書 v1.3・テスト手順書 v1.2（「スマホ縦でフロー完結」は実行すると必ずFAILする合否基準だった）。新規宿題 A-4（mio はアセットのみでシナリオ登場0件）・C-4（テスト系文書の「第1章5本」前提の扱いは102TC手動テストの方針とセットで判断）
...
- 🔄 テスト計画書 **v1.2 起案・POレビュー待ち**（`docs/テスト計画書_v0.1.md`・2026-06-13 v1.0／2026-06-26 v1.1：§10.7 E2E自動化を全10項目に確定・§10.4 分岐網羅方針／**2026-08-25 v1.2：AC-011 の横持ち化を反映**＝要件定義書 v1.1 との連鎖改版）
- ✅ **テスト仕様書 v1.2 承認済み**（2026-06-25・2ファイル構成：本体`テスト仕様書_v0.1.md`§1〜§4＋`テスト仕様書_v0.1_§5-7.md`§5〜§9〔ファイル名は§5-7のままだが内容は§5〜§9〕。v1.0 PO承認→v1.1 非王道ルート TC-005 追加→v1.2 手順書作成時のTD補完 TD-SCN-012〔解説400字〕追加。§8・§9 は逆戻りからの復元版。環境安定時に1ファイルへ統合予定）。**v1.3 起案・POレビュー待ち**（2026-08-25：AC-011 の横持ち化・端末条件を反映）
- ✅ **テスト手順書 v1.1 承認済み**（2026-06-26 v1.0承認／2026-06-27 v1.1：バッジ色を確定色コードに更新。5ファイル構成：本体`テスト手順書_v0.1.md`〔基盤＋FR-006〕＋`_FR001-003`＋`_FR004-008`＋`_FR009`＋`_§5`。テスト仕様書 v1.2 の全102TC〔FT＋ST〕＋CQ を網羅）。**v1.2 起案・POレビュー待ち**（2026-08-25：ST-UB-001-TC-001 を横持ち＋縦持ちの回転案内確認に更新・TC数は102のまま）
- 🔄 **実装（Phase 1）進行中**（足場＋utils層＋単体テスト完了＝2026-06-26）
  ...
  - ✅ **設計書のレイアウト記述と実装の乖離を解消（2026-08-21・設計書 v1.5・宿題C-2）**：§8.4 を実測値へ置換／未実装だったモバイル縦レイアウト表を削除（§8 の回転ガードと矛盾していた）／§9 の依存も実装値へ更新
    - 2026-07-02 立ち絵表示をPO承認で調整：高さ58vh→**68vh**・2体時は**左右分散（justify-around・1体時は中央）**（`ScenarioPlayer.tsx`）。→ **設計書 v1.5（2026-08-21）で §8.4 に反映済み**
...
- 📋 **保留・要対応の宿題は `docs/宿題リスト_v1.0.md` に集約**（A=PO判断・C-1=✅完了／残: B-1本格アート・B-2 vite8更新・B-3 TD実データ・D ロードマップ）
  - ✅ dev依存の脆弱性は解消（2026-08-21・B-2＝`npm audit` 7件→0件。vite 8 / vitest 4 / plugin-react 6 へ更新）
- ✅ **開発フロー・役割分担定義（TestQuest版）v1.0 承認済み**（2026-08-21）：`docs/開発フロー_役割分担定義_v0.1.md`（15章）。出典＝クロスワード `開発フロー_役割分担定義_v9.0.md`（**ファイル名はv9.0だが中身はv10.1**・`C:\Users\gate1\OneDrive - 株式会社閂\AI_WG\crossword\jstqb-crossword-starter\docs\`）。v10.1の13章を全網羅したうえで①**制作フロー3系統を新設**（§5シナリオ／§6画像／§7スキル・ルール改訂）②**§8 検証体制を実態ベースに置換**（6層＝型/単体153/E2E15/実画面/監修全数/通しプレイ＋データ品質門番＋本番スモーク。テストログCSV・DEF管理は §9.2「未稼働の枠」＝**PO決定 2026-08-21**）③§13 データ品質をシナリオJSON中心に再構成（全数監修）④TestQuest固有の承認ルール（監修・画像採用・課金API・push・スキル改訂）⑤基本方針に3原則追加（決定的処理＞生成的処理・既存分不変・実体検証）
...
- ✅ **B-4① 類似性逆引き＋権利チェック 完了（2026-08-21）**：設定書§7 の4項目を**採用済み全27枚**（立ち絵5キャラ×5表情＋背景2）で実施。**Yandex の URL 逆引き**（`yandex.com/images/search?rpt=imageview&url=<本番URL>`＝WebP本番URLをそのまま使えるためアップロード不要。TinEye/Google Lens は JS必須で自動実行不可）で**完全一致0件・特定既存キャラへの酷似なし**。規約版（Gemini API Additional Terms・Effective 2026-03-23/Updated 2026-04-28 UTC）と確認日を記録／プロンプト監査で固有名混入0／文字・署名の写り込みなし（背景は等倍5領域）。**結果に出た固有名4ケースは生成プロンプト文字列・絵柄カテゴリ近傍で酷似の根拠にならないと判定根拠つきで記録**（採用記録 v0.7・worklog 追記46）
- 🔲 **B-4② Pollinations系譜の法務確認**：クロスワード側 D2 の結果待ち（外部依存）。確認時は採用記録 v0.7 の逆引き結果を提示する
- 🔲 **残タスク**：通しプレイFB反映（確認シート回収待ち・**改訂後の再プレイ推奨**）
- 🔲 **通しプレイ総合確認（PO実施中）**：`docs/通しプレイ確認シート_v0.1.md`（全39本・観点C1-C6/A1-A5/X1-X5）。フィードバック受領後にAIが修正案リスト化
- ✅ **AL実装の残作業＝3項目すべて完了**（2026-08-21 実体確認で判明。TODO の記載が古かっただけ）
  - ① CharacterId の 'takumi'/'mio' 追加＝**実装済み**（`src/types/index.ts:21`・`src/utils/validator.ts:13`〔2026-07-03 追加〕）。level 'AL-TM'/'AL-TTA' も `types/index.ts:33`・`utils/levels.ts` で対応済み・index.json の level で全71本が本番配信中
  - ② server 背景＝**完了**（`public/images/backgrounds/server.webp`・採用記録に2026-07-04 PO採用）
...
public/
  ...
  audio/             キャラクターボイス（FR-P2-006・Phase 2）＝`{scenarioId}/{nodeId}.m4a`＋`manifest.json`。**AAC**（Ogg Opus は iOS 16 以前の Safari 非対応）。wav原本は assets-candidates/voice-wav/（gitignore）
tests/e2e/           Playwright E2E（walkthrough・persistence・orientation・select-collapse・voice）
scripts/             画像生成・運用スクリプト（gen-expression-gemini.ps1・remove-bg.mjs・compress-images.mjs 等）＋音声（gen-voice.mjs・gen-voice-dict.mjs・voice-audition.mjs〔STEP 0 の試聴・規約取得〕・voice-cast.json・voice-dict.json）
