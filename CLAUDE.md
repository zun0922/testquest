...
- 🔄 **キャラクターボイス（FR-P2-006・Phase 2）＝全71本の音声生成完了・PO体感確認待ち**（2026-08-25 社内FB→PO承認プラン／2026-08-30 STEP 3 完了）：方式＝**VOICEVOX 0.25.2 CPU版**（無料・CPU推論・決定的生成・ユーザー辞書で読みを制御）
  ...
  - **残＝STEP 2（PO体感確認：読み間違い・話速・クリック挙動 A-6・疑似コード5件の読み A-7）**。読み間違いは**辞書追加＋`--force` で該当分のみ鳴らし直せる**（費用ゼロ）
  - 🔲 **26MBの音声を git にコミットするかはPO判断**（画像は2MB。バイナリのため履歴の圧縮が効かない）
