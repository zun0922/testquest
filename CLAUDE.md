...
- 🔄 **ヒント機能（FR-P2-007）＝全章展開完了・PO確認待ち**（2026-08-30 PO提案→要件確定→実装→全章展開）：蓄積ステータスが何にも使われていない課題への対応。**選択肢の重要語を強調**するヒントを**4種の合計ポイント**に応じて**任意表示**する。詳細の正＝`docs/ヒント機能_要件仕様_v0.1.md`（実測：開始40／best は50本目で上限400／poor は71本で183）。**`Choice.emphasis` を追加し `text` は不変**・章単位で段階導入可。**未決4点はPO判断済み**（宿題A-8）
